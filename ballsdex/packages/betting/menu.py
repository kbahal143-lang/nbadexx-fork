...
from .display import fill_bet_embed_fields
...
class BetMenu:
    ...
    async def cancel(self, reason: str = "The bet has been cancelled."):
        ...
        # Unlock all NBA proposals so players can use them again
        for bettor in (self.bettor1, self.bettor2):
            for nba in bettor.proposal:
                try:
                    await nba.unlock()
                except Exception:
                    log.warning(f"Failed to unlock NBA {nba.pk} during bet cancel", exc_info=True)
    ...
    async def confirm(self, bettor: BettingUser) -> bool:
        ...
        if self.bettor1.accepted and self.bettor2.accepted:
            ...
            # Transfer loser's NBAs to winner and unlock all NBAs
            try:
                from tortoise.transactions import in_transaction
                async with in_transaction():
                    for nba in loser.proposal:
                        await nba.refresh_from_db()
                        if nba.player_id != loser.player.pk or nba.deleted:
                            raise ValueError(f"NBA {nba.pk} is no longer valid for betting")
                        nba.player = winner.player
                        nba.locked = None  # type: ignore
                        await nba.save(update_fields=["player_id", "locked"])
                    for nba in winner.proposal:
                        await nba.refresh_from_db()
                        nba.locked = None  # type: ignore
                        await nba.save(update_fields=["locked"])
            except Exception as e:
                log.error(f"Error transferring NBAs: {e}")
                # Unlock everything on failure
                for bettor in (self.bettor1, self.bettor2):
                    for nba in bettor.proposal:
                        try:
                            await nba.unlock()
...
class BallsSelector(Pages):
    ...
    async def confirm_button(
        ...
        grammar = "NBA" if len(locked_balls) == 1 else "NBAs"
        msg = f"{len(locked_balls)} {grammar} added to your proposal."
        if failed_lock:
            msg += f"\n{len(failed_lock)} NBA(s) could not be added (already locked or unavailable)."
