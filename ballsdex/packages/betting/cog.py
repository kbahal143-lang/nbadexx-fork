...
class Bet(commands.GroupCog):
    ...
    async def add(
        ...
        # Verify NBA belongs to the user
        if nba.player.discord_id != interaction.user.id:
            await interaction.followup.send(
                "You do not own this NBA.", ephemeral=True
            )
            return
        ...
        if await nba.is_locked():
            await interaction.followup.send(
                "This NBA is currently locked (in a trade or another bet). "
                "Please wait for it to be unlocked.",
                ephemeral=True,
            )
            return
        ...
        # Lock the NBA so it cannot be sold or traded while in the bet
        await nba.lock_for_trade()
    ...
    async def remove(
        ...
        if nba in bettor.proposal:
            ...
            # Unlock the NBA so it can be used elsewhere
            try:
                await nba.unlock()
            except Exception:
                log.warning(f"Failed to unlock NBA {nba.pk} on bet remove", exc_info=True)
