...
class CoinsAdmin(app_commands.Group):
    ...
    async def remove(
        ...
        money, _ = await PlayerMoney.get_or_create(player=player)
        old_balance = money.coins
        money.coins = max(0, money.coins - amount)
        await money.save(update_fields=["coins"])
    ...
    async def check(
        ...
        await interaction.response.send_message(
            f"{user.mention} has **{money.coins:,}** coins.",
            ephemeral=True
        )
