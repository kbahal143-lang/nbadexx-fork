...
class Pack(models.Model):
    ...
    special_chance = fields.BooleanField(
        ...
        description=(
            ...
            "Off: every card is guaranteed a special (1 allowed = that one; multiple = equal weight random pick)"
        )
    )
    special_only = fields.BooleanField(
        default=False,
        description="Filter ball pool to only balls that have an existing instance with one of the allowed specials"
    )
