...
def _pick_special_by_rarity(pool: list[Special]) -> Special | None:
    """Pick a special from the pool weighted by rarity. Returns None if pool is empty."""
    if not pool:
        return None
    total = sum(s.rarity for s in pool)
    if total <= 0:
        return random.choice(pool)
    roll = random.uniform(0, total)
    cumulative = 0.0
    for sp in pool:
        cumulative += sp.rarity
        if roll <= cumulative:
            return sp
    return pool[-1]
...
class Packs(commands.GroupCog, group_name="pack"):
    ...
    async def list(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description="Here are the packs you can buy with coins:",
        ...
        try:
            async with in_transaction():
                ...
                if pack.quantity < amount:
                    await interaction.response.send_message(
                        "You no longer have enough packs!",
                        ephemeral=True
                    )
                    return
    ...
    async def open(
        ...
        try:
            ...
            async with in_transaction():
                ...
                # special_chance=True + allowed set  → only those specials, by rarity weight
                # special_chance=False + 1 allowed   → guaranteed that one special
                # special_chance=False + multiple    → equal weight random pick per card
                if the_pack.special_chance and not allowed_specials:
                    special_pool: list[Special] = list(
                        await Special.filter(hidden=False).all()
                    )
                else:
                    special_pool = allowed_specials

                # --- Ball pool ---
                # special_only=True + allowed_specials set → only balls with an instance of those specials
                ...
                results: list[BallInstance] = []

                for _ in range(amount):
                    pack_cards: list[BallInstance] = []

                    for _ in range(the_pack.cards_count):
                        # Select ball by rarity weight
                        ...
                        # Select special for this card
                        special_to_assign: Special | None = None
                        if the_pack.special_chance:
                            # Probability-based: rarity of each special = % chance weight
                            if special_pool:
                                total_sp = sum(s.rarity for s in special_pool)
                                sp_roll = random.uniform(0, 1)
                                if sp_roll < total_sp:
                                    special_to_assign = _pick_special_by_rarity(special_pool)
                        else:
                            # Guaranteed special
                            if len(allowed_specials) == 1:
                                special_to_assign = allowed_specials[0]
                            elif len(allowed_specials) > 1:
                                special_to_assign = random.choice(allowed_specials)
            ...
            if len(results) == 1:
                ...
                sp_display = inst.special_emoji(self.bot).strip()
                special_text = f" {sp_display}" if sp_display else ""
